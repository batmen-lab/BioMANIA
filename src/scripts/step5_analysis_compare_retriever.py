...
def create_subplot_bar_graphs(df, ax, lib, title, colors, display_test=False):
    ...
    for i, col_prefix in enumerate(['BM25', 'Un-Finetuned', 'Finetuned']):
        ...
        ax.text(indices[i] - 3*bar_width/2, val_acc, f'{val_acc:.2f}', ha='center', va='bottom', fontsize=font_size)
        if display_test:
            ax.text(indices[i] - bar_width/2, test_acc, f'{test_acc:.2f}', ha='center', va='bottom', fontsize=font_size)
        ax.text(indices[i] + bar_width/2, val_am_acc, f'{val_am_acc:.2f}', ha='center', va='bottom', fontsize=font_size)
        if display_test:
            ax.text(indices[i] + 3*bar_width/2, test_am_acc, f'{test_am_acc:.2f}', ha='center', va='bottom', fontsize=font_size)
...
if num_libs <= 2:
    ...
    fig, axs = plt.subplots(1, num_libs, figsize=(12 * num_libs, 5), sharey=True) # constrained_layout=True, 
    if num_libs == 1:
        axs = np.array([axs])
else:
    ...
    num_cols = 2
    num_rows = math.ceil(num_libs / num_cols)
    fig, axs = plt.subplots(num_rows, num_cols, figsize=(12, 9), sharey=True) # num_rows * , constrained_layout=True,
    axs = axs.ravel()
...
plt.savefig('output/step5_analysis_compare_retriever.png')
plt.show()
