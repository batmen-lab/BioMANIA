...
logger.remove()
logger.add(log_filename, rotation="500 MB", retention="7 days", level="INFO")
...
if torch.cuda.is_available():
    ...
    logger.info("Current GPU Index: {}", torch.cuda.current_device())
else:
    device = torch.device('cpu')
    logger.info("Current Using CPU")
# 
...
class Model:
    ...
    def load_bert_model(self, load_mode='unfinetuned_bert'):
        self.bert_model = SentenceTransformer('all-MiniLM-L6-v2', device=device) if load_mode=='unfinetuned_bert' else SentenceTransformer(f"./hugging_models/retriever_model_finetuned/{self.LIB}/assigned", device=device)
    def reset_lib(self, lib_name):
        #lib_name = lib_name.strip()
        logger.debug("================")
        logger.debug("==>Start reset the Lib {}!", lib_name)
        ...
        try:
            ...
            logger.info("==>loaded API json done")
            self.load_bert_model()
            logger.info("==>loaded finetuned bert for chitchat")
            ...
            logger.info("load retrieval_model_path in: {}", retrieval_model_path)
            ...
            logger.info('==>Successfully loading model!')
            logger.info("loading model cost: {} s", str(time.time()-t1))
            reset_result = "Success"
            self.LIB = lib_name
        except Exception as e:
            logger.error("at least one data or model is not ready, please install lib first!")
    ...
    def retrieve_names(self,query):
        ...
        logger.info("retrieved_names: {}", retrieved_names)
    ...
    def save_state(self):
        ...
        logger.info("State saved to {}", file_name)
    def load_state(self, session_id):
        ...
        logger.info("State loaded from {}", file_name)
    def run_pipeline(self, user_input, lib, top_k=3, files=[],conversation_started=True,session_id=""):
        ...
        if True:
            ...
            try:
                ...
                logger.error('no local session_id environment exist! start from scratch')
                self.initialize_executor()
                pass
        ...
        if lib!=self.LIB:
            ...
            if reset_result=='Fail':
                logger.error('Reset lib fail! Exit the dialog!')
                return 
        ...
        if conversation_started in ["True", True]:
            logger.info('==>new conversation_started!')
            self.user_states="run_pipeline"
            self.initialize_executor()
            self.executor.variables={}
            self.executor.execute_code=[]
            ...
            logger.info('==>old conversation_continued!')
        if self.user_states == "run_pipeline":
            ...
            logger.info('----query inferred as {}----', predicted_source)
            ...
            for _ in range(self.predict_api_gpt_retry):
                try:
                    ...
                    logger.info('self.all_apis_json keys: {}', self.all_apis_json.keys())
                    logger.info('response in self.all_apis_json: {}', response in self.all_apis_json)
                    self.all_apis_json[response]
                    self.predicted_api_name = response 
                    success = True
                    break
                except Exception as e:
                    logger.error('error during api prediction: {}', e)
            if not success:
                self.initialize_tool()
                self.callback_func('log', "GPT can not return valid API name prediction, please redesign your prompt.", "GPT predict Error")
                return
            logger.info('length of ambiguous api list: {}',len(self.ambiguous_api))
    ...
    def handle_unknown_state(self, user_input):
        logger.info("Unknown state: {}", self.user_states)
    ...
    def run_pipeline_after_ambiguous(self,user_input):
        logger.info('==>run_pipeline_after_ambiguous')
    ...
    def run_pipeline_after_fixing_API_selection(self,user_input):
        logger.info('==>run_pipeline_after_fixing_API_selection')
        ...
        logger.info('self.relevant_api_list: {}', self.relevant_api_list)
        self.api_name_json = self.check_and_insert_class_apis(self.API_composite, self.relevant_api_list)# also contains class API
        logger.info('self.api_name_json: {}', json.dumps(self.api_name_json))
        ...
        logger.info('summary_prompt: {}', summary_prompt)
        response, _ = LLM_response(self.llm, self.tokenizer, summary_prompt, history=[], kwargs={})
        logger.info('summary_prompt response: {}', response)
        self.initialize_tool()
    ...
    def run_pipeline_after_doublechecking_API_selection(self, user_input):
        logger.info('==>run_pipeline_after_doublechecking_API_selection')
        ...
        for api in self.api_name_json:
            ...
            if (maybe_instance_name in self.executor.variables) and (self.API_composite[api]['api_type'] in ['class', 'unknown']):
                # logger.info('skip parameters for {}',maybe_instance_name)
                continue
            else:
                pass
        ...
        try:
            ...
            logger.info('parameters_prompt: {}', parameters_prompt)
        except Exception as e:
            logger.error('error for parameters: {}', e)
        if len(parameters_name_list)==0:
            ...
            for _ in range(self.param_gpt_retry):
                try:
                    ...
                    logger.info('==>Asking GPT: {}, ==>GPT response: {}', parameters_prompt, response)
                    ...
                    pass
                    #return # 231130 fix 
            logger.info('success or not: {}', success)
            if not success:
        ...
        logger.info('finished generate api calling')
        ...
        logger.info("Automatically selected params for $, after selection the parameters are: {}", json.dumps(self.selected_params))
        ...
        logger.info('none_dollar_value_params: {}', json.dumps(none_dollar_value_params))
        if none_dollar_value_params:
            #logger.info(self.user_states)
            self.initialize_tool()
        ...
        if multiple_dollar_value_params:
            logger.info('==>There exist multiple choice for a special type parameters, start selecting parameters')
    ...
    def run_select_special_params(self, user_input):
        logger.info('==>run_select_special_params')
    ...
    def run_pipeline_after_select_special_params(self,user_input):
        if self.last_user_states == "run_select_special_params":
            self.selected_params = self.executor.makeup_for_missing_single_parameter_type_special(params = self.selected_params, param_name_to_update=self.last_param_name, user_input = user_input)
        # @ param
        logger.info('starting entering basic params')
        ...
        if none_at_value_params: # TODO: add type PathLike
            logger.info('if exist non path, basic type parameters, start selecting parameters')
            tmp_input_para = ""
            for idx, api in enumerate(self.filtered_params):
                if idx!=0:
                    tmp_input_para+=" and "
    ...
    def run_select_basic_params(self, user_input):
        logger.info('==>run_select_basic_params')
        ...
        self.initialize_tool()
        logger.info('self.filtered_params: {}', json.dumps(self.filtered_params))
        if len(self.filtered_params)>1:
            self.last_param_name = list(self.filtered_params.keys())[0]
    ...
    def run_pipeline_after_entering_params(self, user_input):
        if self.last_user_states == "run_select_basic_params":
            self.selected_params = self.executor.makeup_for_missing_single_parameter(params = self.selected_params, param_name_to_update=self.last_param_name, user_input = user_input)
        logger.info('==>run pipeline after entering parameters')
        self.update_user_state("run_pipeline")
        self.image_file_list = self.update_image_file_list()
        ...
        logger.info('self.selected_params:')
        logger.info(json.dumps(self.selected_params))
        ...
        logger.info('==>self.api_name_json: {}, parameters_list: {}', self.api_name_json, parameters_list)
        logger.info('==>extracted_params: {}', extracted_params)
        extracted_params_dict = {api_name: extracted_param for api_name, extracted_param in zip(self.api_name_json, extracted_params)}
        logger.info('extracted_params_dict: {}', json.dumps(extracted_params_dict))
        api_params_list = []
            ...
            if True:
                ...
                if self.API_composite[api_name]['api_type'] not in ['class', 'unknown']:
                    ...
                    if len(self.api_name_json)==1:
                        ...
                        pass
        logger.info('==>api_params_list: {}', json.dumps(api_params_list))
        # add optional cards
        optional_param = {key: value for key, value in self.API_composite[api_name]['Parameters'].items() if value['optional']}
        logger.info('==>optional_param: {}', json.dumps(optional_param))
        logger.info('len(optional_param) {}', len(optional_param))
        self.initialize_tool()
        if False: # TODO: if True, to debug the optional card showing
            if len(optional_param)>0:
                logger.info('producing optional param card')
        ...
        logger.info('api_params_list: {}', api_params_list)
        self.execution_code = self.executor.generate_execution_code(api_params_list)
        logger.info('==>execution_code: {}', self.execution_code)
        self.initialize_tool()
    ...
    def run_pipeline_after_doublechecking_execution_code(self, user_input):
        ...
        logger.info('execute and obtain figures')
        ...
        while process.is_alive():
            ...
            with open(temp_output_file, 'r') as file:
                ...
                logger.info('accumulated_output: {}', accumulated_output)
        ...
        logger.info('check: {}, {}, {}, {}', code, output_list, self.executor.execute_code, self.executor.variables)
        ...
        if len(execution_code_list)>0:
            ...
            logger.info('Something wrong with generating code with new API!')
        logger.info('self.executor.variables:')
        logger.info(json.dumps(list(self.executor.variables.keys())))
        logger.info('self.executor.execute_code:')
        logger.info(json.dumps(self.executor.execute_code))
        try:
            content = '\n'.join(output_list)
        except:
            content = ""
        ...
        if self.last_execute_code['success']=='True':
            ...
            if len(vari)>1:
                ...
                if (vari[0] in self.executor.variables) and ((vari[0].startswith('result_')) or (vari[0].endswith('_instance'))):
                    ...
                    if print_type=='AnnData':
                        logger.info('if the new variable is of type AnnData, ')
                        ...
                        if 'obs' in visual_attr_list:
                            ...
                            logger.info('visualize {} attribute', visual_attr)
                            ...
                            pass
                    try:
                        logger.info('if exist table, visualize it')
                    ...
                    logger.info('Something wrong with variables! success executed variables didnt contain targeted variable')
                tips_for_execution_success = False
            else:
                pass
            logger.info('if generate image, visualize it')
            ...
            if new_file_list:
                for new_img in new_file_list:
                    logger.info('send image to frontend')
            ...
            logger.info('Execution Error: {}', content)
        ...
        if self.last_execute_code['success']=='True':
            ...
            pass
        logger.info("Show current variables in namespace:")
        logger.info(json.dumps(list(self.executor.variables.keys())))
        new_str = []
        for i in self.executor.execute_code:
            new_str.append({"code":i['code'],"execution_results":i['success']})
        logger.info("Currently all executed code: {}", json.dumps(new_str))
    ...
    def get_last_execute_code(self, code):
        for i in range(1, len(self.executor.execute_code)+1):
            if self.executor.execute_code[-i]['code']==code:
                ...
                pass
        logger.info('Something wrong with getting execution status by code! Enter wrong code {}', code)
