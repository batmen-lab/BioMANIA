...
import { Message, FileObject } from '@/types/chat';
...
export const ChatInput = ({
  ...
  const handleAddURL = () => {
    ...
    if (url) {
      setAttachedFiles(prevFiles => [...prevFiles, { id: uuidv4(), data: url, type: 'url', filename: "url" }]);
    }
  };
  ...
  return (
    <div className="absolute bottom-0 left-0 w-full border-transparent bg-gradient-to-b from-transparent via-white to-white pt-6 dark:border-white/20 dark:via-[#343541] dark:to-[#343541] md:pt-2">
      <div className="stretch mx-2 mt-4 flex flex-row gap-3 last:mb-2 md:mx-4 md:mt-[52px] md:last:mb-6 lg:mx-auto lg:max-w-3xl">
        ...
        <div className="relative mx-2 flex w-full flex-grow flex-col rounded-md border border-black/10 bg-white shadow-[0_0_10px_rgba(0,0,0,0.10)] dark:border-gray-900/50 dark:bg-[#40414F] dark:text-black dark:shadow-[0_0_15px_rgba(0,0,0,0.10)] sm:mx-4">
          {/* File cards list */}
          <div className="file-card-list flex flex-wrap gap-0 mb-[0px]">
            {attachedFiles.map(({ id, data, type, filename }) => {
              if (type === 'file') {
                const file = data as File;
                return (
                  <div 
                    key={id} 
                    className="file-card flex items-center space-x-2 border p-2 bg-gray-100 rounded-lg"
                ...
                return (
                  <div 
                    key={id} 
                    className="url-card flex items-center space-x-2 border p-2 bg-gray-100 rounded-lg"
                    style={{ maxWidth: 'calc(100% - 10px)' }}
                  >
          ...
          {/* File upload button */}
          <button
            className="absolute left-2 bottom-2.5 rounded-sm p-1 text-neutral-800 opacity-60 hover:bg-neutral-200 hover:text-neutral-900 dark:bg-opacity-50 dark:text-neutral-100 dark:hover:text-neutral-200"
            onClick={() => fileInputRef.current?.click()}
          >
            <IconPlus size={18} />
          </button>
          <button
            className="absolute left-8 bottom-2.5 rounded-sm p-1 text-neutral-800 opacity-60 hover:bg-neutral-200 hover:text-neutral-900 dark:bg-opacity-50 dark:text-neutral-100 dark:hover:text-neutral-200"
            onClick={() => {
              const url = prompt('Please enter the URL:');
              if (url) {
                setAttachedFiles(prevFiles => [...prevFiles, { id: uuidv4(), data: url, type: 'url', filename: "url" }]);
          ...
          <textarea
            ref={textareaRef}
            className="m-0 w-full left-16 resize-none border-0 bg-transparent p-0 py-2 pr-8 pl-18 text-black dark:bg-transparent dark:text-white md:py-3 md:pl-28"
            style={{
              resize: 'none',
          ...
          />
          <button
            className="absolute right-2 bottom-2.5 rounded-sm p-1 text-neutral-800 opacity-60 hover:bg-neutral-200 hover:text-neutral-900 dark:bg-opacity-50 dark:text-neutral-100 dark:hover:text-neutral-200"
            onClick={handleSend}
          >
            {messageIsStreaming ? (
      ...
      <div className="px-3 pt-2 pb-3 text-center text-[12px] text-black/50 dark:text-white/50 md:px-4 md:pt-3 md:pb-6">
        {t(
          "BioMANIA UI is an interface based off of ",
        )}
        <a
